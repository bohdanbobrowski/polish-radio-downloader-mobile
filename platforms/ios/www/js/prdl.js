      ...
      success: function(page_html) {
         ...
         $$('#menu_podstrony_'+page.id+' a.prdl_media').on('click', function (e) {
            ...
            downloadmanager(
               ...
               {
                  url: file_url,
                  filePath: "PolskieRadioDownloader",
                  fileName: file_name,
                  overwrite: false,
                  useNotificationBar: true,
                  startToast: "Pobieranie: '"+file_name+"'",
                  endToast: "Pobrano: '"+file_name+"'",
                  cancelToast: "Pobieranie anulowano!"
               },
               function(info) {
...
function getMediaLink(media) {  
   ...
   html = html + '<div class="item-content"><div class="item-media"><i class="icon icon-volume"></i></div>';
